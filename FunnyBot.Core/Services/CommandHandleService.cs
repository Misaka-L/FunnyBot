...
namespace FunnyBot.Core.Services {
    public class CommandHandleService : IHostedService {
        ...
        private async Task _commandService_CommandExecuted(CommandInfo commandInfo, ICommandContext context, IResult result) {
            if (!result.IsSuccess && result.Error is CommandError error && error != CommandError.UnknownCommand) {
                await context.Channel.SendCardsAsync(new CommandErrorCardMessage(commandInfo, context, result).Build());
            }
        }

        private async Task _socketClient_MessageReceived(SocketMessage arg, SocketUser socketGuildUser, IMessageChannel arg3) {
